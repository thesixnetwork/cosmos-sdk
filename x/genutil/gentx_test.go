...
func (suite *GenTxTestSuite) SetupTest() {
	...
	suite.msg1, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk1.Address()).String(), pk1.String(), pk1, amount, desc, comm, one)
	suite.NoError(err)
	suite.msg2, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk2.Address()).String(), pk1.String(), pk1, amount, desc, comm, one)
	suite.NoError(err)
}

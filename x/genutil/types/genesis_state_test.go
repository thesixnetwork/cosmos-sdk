...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	msg1, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk1.Address()).String(), pk2.String(), pk1, sdk.NewInt64Coin(sdk.DefaultBondDenom, 50), desc, comm, math.OneInt())
	require.NoError(t, err)

	msg2, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk2.Address()).String(), pk2.String(), pk2,
...
func TestValidateGenesisBadMessage(t *testing.T) {
	...
	msg1 := stakingtypes.NewMsgEditValidator(sdk.ValAddress(pk1.Address()).String(), desc, nil, nil, stakingtypes.ValidatorMode_MODE_NORMAL, &math.Int{}, nil)

...
# Backdate genesis time by 2 days: a validator's commission can only change
# once per 24h and its UpdateTime is stamped with genesis time, so with a
# backdated genesis the cooldown is already over at block 1 and
# `edit-validator --commission-rate` works immediately after startup
...
if [ "$VAL_MODE" = "0" ]; then
  simd genesis gentx bob 1000000000000ustake --min-self-delegation="10000000000" --commission-rate=0.1 --commission-max-rate=0.5 --commission-max-change-rate=0.5 --validator-mode=0 --min-delegation="10000000000" --keyring-backend $KEYRING --chain-id $CHAINID
elif [ "$VAL_MODE" = "1" ]; then
  simd genesis gentx bob 1000000000000ustake --min-self-delegation="1000000000000" --commission-rate=0.1 --commission-max-rate=0.5 --commission-max-change-rate=0.5 --validator-mode=1 --min-delegation="1000000000000" --delegation-increment="1000000000000" --max-license=10 --keyring-backend $KEYRING --chain-id $CHAINID --home ${STAKE_HOME}
elif [ "$VAL_MODE" = "2" ]; then
  simd genesis gentx bob 1000000000000ustake --min-self-delegation="10000000000" --commission-rate=0.1 --commission-max-rate=0.5 --commission-max-change-rate=0.5 --validator-mode=2 --min-delegation="10000000000" --keyring-backend $KEYRING --chain-id $CHAINID
else
